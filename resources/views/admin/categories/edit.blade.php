            ...
            <section class="flex-1 space-y-6">
                <div class="rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-gray-200/80 sm:p-5">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-lg font-semibold text-gray-900">Edit category</p>
                                <p class="text-sm text-gray-500">Update the title for this category.</p>
                            </div>
                        </div>
                ...
                <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
                    <div class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-gray-200/80 sm:p-8">
                        <div class="border-b border-gray-100 pb-5">
                            ...
                            <p class="mt-2 max-w-2xl text-sm text-gray-500">
                                Change the category title and save the updated value.
                            </p>
                        </div>
                        ...
                        <form action="{{ route('category.update', $category) }}" class="mt-6 space-y-6" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="space-y-2">
                                <x-input-label for="title" value="Category title" />
                                <x-text-input
                                    id="title"
                                    name="title"
                                    type="text"
                                ...
                                <p class="text-sm text-gray-500">
                                    Keep the title short and easy to recognize.
                                </p>
                    ...
                    <aside class="rounded-[2rem] bg-slate-900 p-6 text-white shadow-sm sm:p-7">
                        ...
                        <h3 class="mt-4 text-xl font-semibold">{{ $category->title }}</h3>
                        <p class="mt-3 text-sm leading-6 text-slate-300">
                            Update the title if the category name should appear differently in the admin panel or on the site.
                        </p>
                        ...
                        <div class="mt-6 rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
                            ...
                            <p class="mt-2 text-sm font-medium text-white">{{ $category->created_at?->format('d.m.Y H:i') }}</p>
                        </div>
                    </aside>
                </div>
            </section>
