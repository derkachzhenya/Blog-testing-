                ...
                <div class="rounded-[2rem] bg-white p-4 shadow-sm ring-1 ring-gray-200/80 sm:p-5">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-lg font-semibold text-gray-900">Create post</p>
                                <p class="text-sm text-gray-500">Add a new post with content, category, and tags.</p>
                            </div>
                        </div>
                ...
                <div class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]">
                    <div class="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-gray-200/80 sm:p-8">
                        <div class="border-b border-gray-100 pb-5">
                            ...
                            <p class="mt-2 max-w-2xl text-sm text-gray-500">
                                Fill in the required title and content. Category and tags are optional.
                            </p>
                        </div>

                        <form action="{{ route('post.store') }}" class="mt-6 space-y-6" method="POST">
                            @csrf
                            ...
                            <div class="grid gap-6 lg:grid-cols-2">
                                ...
                                <div class="space-y-2">
                                    ...
                                    >
                                        @foreach ($tags as $tag)
                                            <option value="{{ $tag->id }}" @selected(in_array((string) $tag->id, old('tags', []), true))>
                                                {{ $tag->title }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="flex flex-col gap-3 border-t border-gray-100 pt-6 sm:flex-row sm:items-center sm:justify-between">
                                <p class="text-sm text-gray-500">
                                    The post will be saved immediately after submission.
                                </p>
                    ...
                    <aside class="rounded-[2rem] bg-slate-900 p-6 text-white shadow-sm sm:p-7">
                        ...
                        <p class="mt-3 text-sm leading-6 text-slate-300">
                            A post stores a title, body content, an optional category, and optional tags connected through the post_tags table.
                        </p>

                        <div class="mt-6 space-y-3 rounded-[1.5rem] border border-white/10 bg-white/5 p-4">
                            <div class="rounded-2xl bg-white/10 px-4 py-3">
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Required</p>
                                <p class="mt-2 text-sm font-medium text-white">title, content</p>
                            </div>

                            <div class="rounded-2xl bg-white/10 px-4 py-3">
                                <p class="text-xs uppercase tracking-[0.2em] text-slate-400">Optional</p>
                                <p class="mt-2 text-sm font-medium text-white">category, tags</p>
                            </div>
                        </div>
                    </aside>
                </div>
